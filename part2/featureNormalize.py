...
def featureNormalize(X):
    ...
    # Вычисляем среднее каждого признака
    mu = np.mean(X, axis=0)

    # Вычитаем среднее из каждого признака
    X_norm = X - repmat(mu, X.shape[0], 1)

    # Вычисляем среднеквадратическое отклонение для каждого признака
    sigma = np.std(X, axis=0, ddof=1)

    # Делим нормализованную матрицу на стандартное отклонение
    X_norm = X_norm / repmat(sigma, X.shape[0], 1)
